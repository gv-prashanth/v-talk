	...
	if (resp.length > 0) {
		...
		for (var i = 0; i < resp.length; i++) {
			...
			if (resp[i].message.includes("base64"))
				...
				constructedString = '<div class="square" ' + styleString + '> <div class="repoDesc"> <p>' + postProcess(resp[i].message, resp[i].sender.toUpperCase()) + '</p> </div> <div style="float:right;"><p>' + timeSince(resp[i].createdOn.seconds);
				if (globalSender.toLowerCase() != resp[i].sender.toLowerCase())
